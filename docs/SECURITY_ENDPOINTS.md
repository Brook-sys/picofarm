...
Sales-channel routes are designed in `docs/SALES_CHANNELS.md`. Any new generic `/api/sales-channels/*` endpoint that connects accounts, handles OAuth/API keys, syncs external data, links products, updates inventory, processes orders, or receives webhooks belongs in this section and should follow the same secret-redaction and fake-client testing rules as the provider-specific routes below. Mercado Livre uses OAuth bearer/refresh tokens and test users rather than a separate sandbox, so CI must rely on fake clients/fixtures and manual QA must avoid personal or production accounts. Shopee uses OAuth shop authorization plus signed V2 API requests; partner keys, OAuth codes, access/refresh tokens, HMAC base strings containing token/shop context, signed URLs, and webhook signatures must be treated as secrets and replaced with `[REDACTED]` in logs, docs examples, API responses, and sync-run errors. OLX Brasil OAuth codes, client secrets, access tokens, lead endpoint tokens, phone/email lead payloads, and ad import payloads must be redacted and tested with fake fixtures; do not implement scraping-based collection.
