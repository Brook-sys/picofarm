...
- Shopee MVP implementation: keep descriptor plus `orders_read`/`products_read` only, keep `inventory_write` and `webhooks` hidden until their gates pass, validate item/model/SKU mapping and idempotent canonical order/product sync with fake fixtures, and verify pagination/rate-limit errors are sanitized through sync-run diagnostics.
