...
- Shopee MVP implementation: start with descriptor plus `orders_read`/`products_read` only, keep `inventory_write` and `webhooks` hidden until their gates pass, validate item/model/SKU mapping with fake fixtures, and verify pagination/rate-limit errors are sanitized through sync-run diagnostics.
