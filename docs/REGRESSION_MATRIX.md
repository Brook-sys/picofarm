...
- Sales-channel connect/auth change: test unsupported capability paths, missing provider-specific fields, OAuth state mismatch/replay where applicable, sanitized callback errors, and redaction for `api_key`, `client_secret`, `code`, access/refresh tokens, webhook secrets, and signed headers. Mercado Livre QA must use fake-client fixtures in CI and optional test users for manual checks; there is no separate sandbox.
