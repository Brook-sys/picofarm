...
- OLX implementation: validate official integrator access and target category/scope before code, use only official APIs or manual/import fallback, do not advertise `orders_read` unless order semantics are documented for the account, and cover OAuth/ad-import/lead-webhook paths with fake fixtures plus secret/PII redaction.
