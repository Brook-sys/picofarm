...
function ProfileSection({ category, title, description, items, onAdd, onUpload, onEdit, uploading }: { category: ProfileCategory; title: string; description: string; items: Array<Record<string, unknown>>; onAdd: () => void; onUpload: (files?: FileList | null) => void; onEdit: (name: string) => void; uploading: boolean }) {
  return <div className="card p-5"><div className="flex items-start justify-between gap-3 mb-4"><div><h3 className="font-semibold text-surface-100">{title}</h3><p className="text-xs text-surface-500 mt-1">{description}</p></div><div className="flex gap-2"><label className="btn btn-secondary text-xs py-1.5 px-3 cursor-pointer"><Upload className="h-3.5 w-3.5 mr-1" /> {uploading ? 'Uploading...' : 'Upload JSON'}<input type="file" accept=".json,application/json" multiple className="hidden" onChange={e => { onUpload(e.target.files); e.currentTarget.value = '' }} /></label><button onClick={onAdd} className="btn btn-secondary text-xs py-1.5 px-3"><Plus className="h-3.5 w-3.5 mr-1" /> Import</button></div></div><div className="space-y-3">{items.map((item, index) => <div key={`${category}-${String(item.name)}-${index}`} className="rounded-xl border border-surface-800 bg-surface-900/60 p-3"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><div className="font-medium text-surface-100 truncate">{String(item.name || 'Unnamed')}</div><div className="mt-1 text-xs text-surface-500">{item.size ? `${String(item.size)} bytes` : 'profile'}{item.updatedAt ? ` · ${String(item.updatedAt)}` : ''}</div></div>{item.name ? <button onClick={() => onEdit(String(item.name))} className="btn btn-secondary text-xs py-1.5 px-2"><Pencil className="h-3.5 w-3.5 mr-1" /> Edit</button> : null}</div>{typeof item.sourceUrl === 'string' && item.sourceUrl.length > 0 ? <a href={String(item.sourceUrl)} target="_blank" rel="noreferrer" className="mt-2 inline-flex items-center text-xs text-blue-300 hover:text-blue-200"><ExternalLink className="h-3 w-3 mr-1" /> Source</a> : null}</div>)}{items.length === 0 && <div className="text-center py-6 text-surface-500 text-sm">No {title.toLowerCase()} loaded</div>}</div></div>
}
